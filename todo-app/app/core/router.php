<?php
//many thanks to you : Ismael Azaran
namespace App;
...
class Router{
    ...
    {
        if ($method === 'map') {
            $maps = array_map('strtoupper', $params[0]); //sets params[0] to uppercase
            $uri = strpos($params[1], '/') === 0 ? $params[1] : '/' . $params[1]; //find  a needle in the haystack
            $callback = $params[2]; //this is our callback
        } else {
            $maps = null;
